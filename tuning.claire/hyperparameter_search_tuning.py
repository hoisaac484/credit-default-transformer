...
def run_sweep(n_trials: int = 30, max_epochs: int = 25,
    ...
    # Load once at batch_size=128 (smallest in search); trials with batch_size=256
    # will reload. pos_weight is independent of batch size so compute it once.
    _, _, _, pos_weight, _ = get_dataloaders(batch_size=256, seed=seed)
    ...
    for i in range(n_trials):
        ...
        train_loader, val_loader, _, _, _ = get_dataloaders(
            batch_size=hp["batch_size"], seed=seed
        )
